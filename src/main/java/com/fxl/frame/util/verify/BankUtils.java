...
public class BankUtils {
    ...
    public static Map<String, String> getBankList(Integer pageNo, int pageSize){
    	Map<String, String> map = new HashMap<String, String>();
    	int endIndex = pageNo * pageSize;
    	if(endIndex > bankBin.length){
    		endIndex = bankBin.length;
    	}
    	long[] pageBin = Arrays.copyOfRange(bankBin, (pageNo-1)*pageSize, endIndex);
    	for(Long bin : pageBin){
    		char[] cardNumber = bin.toString().toCharArray();
    		String name = BankUtils.getNameOfBank(cardNumber, 0);
    		map.put(bin+"", name);
    	}
    	return map;
    }
    public static int getBankBinLength(){
    	return bankBin.length;
    }
 
    public static void main(String[] args) {
    	/*String number = "622202";
    	char[] cardNumber = number.toCharArray();
        String name = BankUtils.getNameOfBank(cardNumber, 0);// 获取银行卡的信息
        //System.out.println(name.split("\\.")[0]);*/
    	
    	for(Long bin : bankBin){
    		char[] cardNumber = bin.toString().toCharArray();
    		String name = BankUtils.getNameOfBank(cardNumber, 0);
